...
from models.VAE_final import VAE
...
save_dir = '/data/yongzheng/cass_musdb/results/vae/'
...
vae = VAE(NUM_NETWORKS, learning_rate, device, 100)
vae.build_model()
...
for j in range(1, NUM_EPOCHS+1):
    ...
    for i, batch in enumerate(dl):
        ...
        enc_losses, dec_losses = vae.train(x, [a, b])
    ...
    for i, batch in enumerate(dl2):
        IDX = batch.numpy()
        x = stft_mix_test[IDX]
        vocal_results, accom_results = vae.test(x)
    ...
    print("MAX: vocal sdr: {:.4f}, accom sdr: {:.4f}".format(
        vocal_max, accom_max))
    print("Enc Loss = {:.4f}, Dec Loss = {:.4f}".format(enc_losses, dec_losses))
    if curr_error > best_result:
        best_result = curr_error
        counter = 0
        # save model
        vae.save_model(save_dir)
    ...
    for key in vae.history.keys():
        np.save(save_dir + key, vae.history[key])
