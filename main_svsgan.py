...
from models.SVSGAN_final import SVSGAN
...
save_dir = '/data/yongzheng/cass_musdb/results/svsgan/'
...
svsgan = SVSGAN(NUM_NETWORKS, learning_rate, device, 100, weight_decay)
svsgan.build_model()
...
for j in range(1, NUM_EPOCHS+1):
    ...
    for i, batch in enumerate(dl):
        ...
        enc_losses, dec_losses, dis_losses = svsgan.train(x, [a, b])
    ...
    for i, batch in enumerate(dl2):
        IDX = batch.numpy()
        x = stft_mix_test[IDX]
        bass_results, clar_results = svsgan.test(x)
    ...
    curr_error = 0.5*(bass_error + clar_error)
    curr_sdr = 0.5*(bass_sdr + clar_sdr)
    print(10*"=")
    ...
    for key in svsgan.history.keys():
        np.save(save_dir + key, svsgan.history[key])
